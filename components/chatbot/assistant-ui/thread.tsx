...
export const Thread: any = ({
  ...
  const handleApiSuccess = async (data: any) => {
    ...
    await new Promise((resolve) => {
      setTimeout(resolve, 5000); // waits 5 seconds
    });
    setTaxBoxPopUp(true);
  ...
  const taxBoxApi = async (data: any) => {
    try {
      ...
      setHasSubmittedTaxData(true); // Update the state in the parent component
      setTaxBoxPopUp(false); // Close the modal after successful submit
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    if (!hasSubmittedTaxData) {
      setTaxBoxPopUp(true);
      setModalType("taxdata");
    }
  }, [hasSubmittedTaxData]); // Depend on `hasSubmittedTaxData`

  console.log(hasSubmittedTaxData, "hasSubmittedTaxData");
    ...
    <>
      <div>
        ...
        {activeTab === "tax" ? (
          ...
          // the "learn" tab logic remains unchanged
          <ThreadPrimitive.Root
            className="bg-background box-border flex flex-col overflow-hidden"
            style={{
              ["--thread-max-width" as string]: "42rem",
