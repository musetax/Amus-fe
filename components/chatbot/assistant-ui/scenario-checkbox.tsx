...
export const ScenarioCheckbox: React.FC<ScenarioCheckboxProps> = ({
  ...
  const thread = useThreadRuntime();
  const [selectedScenarios, setSelectedScenarios] = useState<string[]>([]);
  const [spouseIncome, setSpouseIncome] = useState<string>(""); // 🆕 added
  const [payrollData, setPayrollData] = useState<any>(null);
  const [calculating, setCalculating] = useState(false);
  const [completed, setCompleted] = useState(false);
  ...
  const handleCheckboxChange = (scenarioId: string) => {
    setSelectedScenarios((prev) => {
      ...
      // 🆕 Reset spouse income when unchecking "Got Married"
  ...
  const handleCalculate = () => {
    ...
    // 🆕 Validate spouse income if got_married selected
    if (
      ...
      <div className="flex flex-col flex-wrap gap-3">
        {scenarios.map((scenario) => (
          <div key={scenario.id}>
            ...
            {/* 🆕 Spouse income field - only for 'got_married' */}
